...
class Database:
    ...
    def add_video(Name: str, Path: str, Description: str, OwnerLogin: str, Tags: list) -> None:
        ...
        """
        with sqlite3.connect('database.db') as conn:
            conn.execute('INSERT INTO Videos (Name, Path, ImagePath, Description, OwnerId, DateTime, TagsJSON) VALUES (?, ?, ?, ?, ?, ?, ?)', (Name, Path+'.mp4',Path+'.png', Description, OwnerLogin, datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S'), json.dumps(Tags) if Tags else None))
